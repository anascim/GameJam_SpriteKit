   ...
   //shape.position = CGPoint(x: self.frame.midX, y: self.frame.midY)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if level1.contains(touchLocation){
            self.view?.presentScene(GameScene(level: LevelManager.level1, size: view!.bounds.size))
            
        }
        
        if level2.contains(touchLocation){
            self.view?.presentScene(GameScene(level: LevelManager.level2, size: view!.bounds.size))
